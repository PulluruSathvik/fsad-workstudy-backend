...
public class WorkHourController {
    ...
    @GetMapping("/export")
    public org.springframework.http.ResponseEntity<byte[]> exportLedger() {
        List<WorkHour> hours = workHourRepository.findAll();
        byte[] pdfBytes = reportService.generateFinancialReport(hours);

        org.springframework.http.HttpHeaders headers = new org.springframework.http.HttpHeaders();
        headers.setContentType(org.springframework.http.MediaType.APPLICATION_PDF);
        headers.setContentDispositionFormData("filename", "ledger_report.pdf");
